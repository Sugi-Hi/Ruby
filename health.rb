...
def status_health(health)
  ...
  case health[:stress]
  when 1
    puts "[ランク1]予防の対策のみで、いつも通りで平気と考えられます."
  when 2
    puts "[ランク2]趣味を探して、リフレッシュできる機会を増やせると良いです。"
  when 3
    puts "[ランク3]健康推進の生活習慣・食品の購入などを心がけましょう。！"
  when 4    
    puts "[ランク4]ケアプランなど協力してくれるカウンセラーを探し出して下さい！！"
  when 5
...
puts "+*+ヘルスケアとして、下記の受診内容[初期番号0~4]を入力して下さい+*+"
...
option = gets.to_i
  ...
  when 4
    ...
    puts "受診内容の番号が無い為、再度[初期番号0~4]で入力し直して下さい！\n\n"
  end
